...
int total_veiculos;
int veiculos_turno;
int veiculos_numero = 0;
sem_t semaforo_ilha;
sem_t semaforo_continente;
sem_t semaforo_veiculos;
...
void ponte_inicializar() {
	...
	sem_init(&semaforo_veiculos, 0, 1);
	// ToDo: IMPLEMENTAR!
...
void ponte_entrar(veiculo_t *v) {
	...
	sem_wait(&semaforo_veiculos);
	veiculos_numero++;
	sem_post(&semaforo_veiculos);
	// ToDo: IMPLEMENTAR!
	//sem_wait(&semaforo);
